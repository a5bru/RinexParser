...
class RinexQuality(object):
    ...
    def do_prepare_datadict(self, datadict, gapsize):
        ...
        for i in range(len(epochs) - 1):
            epochs_delta.append(
                int(
                    (epochs[i + 1] - epochs[i]).total_seconds()
                )
            )
        ...
        # epochs_gaps = [(ed / datadict["epochInterval"]) for ed in epochs_delta]
        # for i in range(len(epochs_gaps)):
        #     if epochs_gaps[i] > 1:
        #         gap_epoch_count = ed / datadict["epochInterval"]  # epochs_gaps[i] - 1
        #         chkdoy["gaps"].append({
        #             "gap_begin": epochs[i].strftime(cc.RNX_FORMAT_DATETIME),
        #             "gap_end": epochs[i + 1].strftime(cc.RNX_FORMAT_DATETIME),
        #             "gap_epoch_count": gap_epoch_count,
        #             "gap_duration": ed,  # (gap_epoch_count * datadict["epochInterval"])
        #         })
        #         chkdoy["epochs_missing"] += int(gap_epoch_count)
        # gaps_less = len([g for g in epochs_gaps if (g > 1) & (g <= gapsize)])
        # gaps_more = len([g for g in epochs_gaps if (g > gapsize)])
